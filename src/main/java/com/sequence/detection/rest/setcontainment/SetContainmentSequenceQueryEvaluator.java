...
//    public List<Long> verifyPattern(List<Long> candidates, Sequence query){
    public void verifyPattern(List<Long> candidates, Sequence query, String tableName) {
        ArrayList<Long> containQuery = new ArrayList<>();
//        if(candidates.isEmpty()){
//            return containQuery;
//        }
        List<QueryPair> query_tuples = query.getQueryTuplesConcequtive();
        ResultSet rs = session.execute("SELECT " + "events" + " FROM " + cassandra_keyspace_name + "." + tableName + " WHERE sequence_id = ? ", String.valueOf(candidates.get(0)));
        Row row = rs.one();


//       TODO: transfer this to a seperate function
        List<Event> events = new ArrayList<>();
        List<Date> timestamps = new ArrayList<>();

        String test ="Event(2020-08-15 12:00:00,7E2NV1PO)";
        String[] x = test.split("\\(")[1].split("\\)")[0].split(",");
        Date date = new Date();
        try {
        ...
        Sequence s = new Sequence(eventsArray);
